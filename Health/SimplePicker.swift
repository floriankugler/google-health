...
public struct SimplePicker<Item: Hashable, ID: Hashable, V: View>: View {
...
public extension SimplePicker where Item: Identifiable, Item.ID == ID {
...
var sample: some View {
    VStack {
        WithState("Inbox") { $binding in
            SimplePicker(items: ["Inbox", "Two", "Three"], id: \.self, selection: $binding, view: { Text($0) })
        }
        WithState(Mailbox.inbox) { $value in
            SimplePicker(items: Mailbox.allCases, selection: $value) { box in
                switch box {
...
struct SimplePickerPreview: PreviewProvider {
