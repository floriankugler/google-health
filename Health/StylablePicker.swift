...
public struct StylableSimplePicker<Item: Hashable, ID: Hashable, V: View>: View {
    var items: [Item]
...
public extension StylableSimplePicker where Item: Identifiable, Item.ID == ID {
...
struct StylableSimplePicker_Previews: PreviewProvider {
    static var previews: some View {
        WithState("One") { $state in
            let picker = StylableSimplePicker(items: ["One", "Two", "Three"], id: \.self, selection: $state, view: {
                Text($0)
            })
            .padding()
